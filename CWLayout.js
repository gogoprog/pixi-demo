...
export default function CWLayout(nodeSprites, nodeContainer, visualConfig) {
    ...
    // 存储聚类集合
    let clusters = createCluster(nodes, whisper, this.NODE_WIDTH);
    // 存储所有聚类集合中节点数目最多的集合的索引
    let maxIndex = getMaxIndex(clusters);
    // 计算坐标
...
// 组合继承Layout
CWLayout.prototype = new Layout();
CWLayout.prototype.constructor = CWLayout;

// 计算CW坐标
CWLayout.prototype.calCWPosition = function (nodes, clusters, maxIndex, NODE_WIDTH) {
...
CWLayout.prototype.calCWAngle = function (cluster, clusters, treeLevelRadius, treeLevelAngle, levela, NODE_WIDTH) {
...
CWLayout.prototype.moveAngle = function (cluster, clusters, angle, levela) {
...
function createCluster(nodes, whisper, NODE_WIDTH) {
    let clusters = [];
